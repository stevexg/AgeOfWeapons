...
public class ItemHeavyStaffDiamond extends ItemCustomWeapon {

    public ItemHeavyStaffDiamond(ToolMaterial material) {
        super(material, 0.7F);
                ...
                for(d1 = attacker.posZ - target.posZ; d * d + d1 * d1 < 0.0001D; d1 = (Math.random() - Math.random()) * 0.01D)
{
		...
		target.motionY += 0.60000000596046448D;
		target.motionZ -= (d1 / (double)f) * (double)f1;
        	if(target.motionY > 0.60000000596046448D) {
        		target.motionY = 0.60000000596046448D;
        	}
        	return true;
	}
