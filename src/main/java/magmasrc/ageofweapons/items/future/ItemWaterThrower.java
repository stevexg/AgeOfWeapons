...
public class ItemWaterThrower extends Item {
	...
	public void addInformation(ItemStack  stack, EntityPlayer playerIn, List addList, boolean advanced) {
		if(AgeOfWeapons.activateShowAges) {
			addList.add(ChatFormatting.GRAY + "Future Age");
		}
	}
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		...
		playerIn.getCooldownTracker().setCooldown(this, 15);
			...
			if (worldIn.isRemote) {
				worldIn.spawnParticle(EnumParticleTypes.WATER_SPLASH, xCoord, yCoord, zCoord, x * 10, y * 10, z * 10);	
				worldIn.spawnParticle(EnumParticleTypes.WATER_BUBBLE, xCoord, yCoord, zCoord, x * 10, y * 10, z * 10);	
			} else {
				...
				if (worldIn.isBlockFullCube(new BlockPos(xCoord, yCoord, zCoord)) &&
						...
						worldIn.setBlockState(new BlockPos(xCoord, yCoord + 1, zCoord), Blocks.FLOWING_WATER.getDefaultState());
				}
	   ...
	   for(int i = 0; i < 100; i++) {
		   worldIn.spawnParticle(EnumParticleTypes.WATER_SPLASH, playerIn.posX + x * i + random.nextDouble(), playerIn.posY + y * i + random.nextDouble(), playerIn.posZ + z * i + random.nextDouble(), x * 10, y * 10, z * 10);
		   worldIn.spawnParticle(EnumParticleTypes.WATER_BUBBLE, playerIn.posX + x * i + random.nextDouble(), playerIn.posY + y * i + random.nextDouble(), playerIn.posZ + z * i + random.nextDouble(), x * 10, y * 10, z * 10);
	   }
	...
	public boolean getIsRepairable(ItemStack armor, ItemStack stack) {
	 return stack.getItem() == ModItems.tankWater;
	}  
}
